...
def _clean_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("pm.client.sync_venv", lambda *args, **kwargs: None)
...
def _assert_cloud_client_lazy_installed_before_import(tmp_path, monkeypatch, mode: str):
    ...
    monkeypatch.setattr(importlib, "import_module", lambda name, *a, **kw: (
        (_ for _ in ()).throw(ModuleNotFoundError("missing", name=name))
        if name == "hindsight_client" else real_import_module(name, *a, **kw)
    ))
...
class TestPostSetupEnvEncoding:
    def _run_cloud_post_setup(self, tmp_path, monkeypatch):
        ...
        # Skip PM publishing; this test exercises env encoding, not installs.
        monkeypatch.setattr("pm.client.sync_venv", lambda *a, **kw: None)
...
class TestClientAutoUpgradeRoutesThroughPM:
    """Older clients use the selected PM extra, never a direct installer."""

    def _init_with_outdated_client(self, tmp_path, monkeypatch, failure=None):
        ...
        monkeypatch.setattr(subprocess_mod, "run", lambda *a, **kw: pytest.fail("direct pip invoked"))
        provider = HindsightMemoryProvider()
        provider.initialize(session_id="s", hermes_home=str(tmp_path), platform="cli")
        return calls

    def test_upgrade_requests_selected_extra(self, tmp_path, monkeypatch):
        assert self._init_with_outdated_client(tmp_path, monkeypatch) == [["hindsight"]]

    def test_blocked_upgrade_is_nonfatal_and_surfaces_reason(self, tmp_path, monkeypatch, caplog):
        import logging
        with caplog.at_level(logging.WARNING):
            calls = self._init_with_outdated_client(tmp_path, monkeypatch, "runtime installs are disabled")
        assert calls == [["hindsight"]]
        assert "runtime installs are disabled" in caplog.text
        assert "hermes pm install --extra hindsight" in caplog.text
