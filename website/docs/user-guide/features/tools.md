...
| **Memory & recall** | `memory`, `session_search`, `session_list`, `session_read`, `knowledge_search`, `knowledge_read` | Persistent memory, session recall, and local knowledge retrieval. |
...
Common toolsets include `web`, `search`, `terminal`, `file`, `knowledge`, `browser`, `vision`, `image_gen`, `moa`, `skills`, `tts`, `todo`, `memory`, `session_search`, `cronjob`, `code_execution`, `delegation`, `clarify`, `homeassistant`, `messaging`, `spotify`, `discord`, `discord_admin`, `debugging`, `safe`, and `rl`.
