...
def _default_platform_hint(platform_key: str, available_tools=None) -> str:
    """Built-in/plugin platform hint, trimmed to known capabilities when possible."""
    hint = PLATFORM_HINTS.get(platform_key, "")
    if hint and (available_tools or (platform_key == "telegram" and _telegram_rich_messages_enabled())):
        hint = build_platform_hint(platform_key, available_tools)
...
def _cron_delivery_hint(agent: Any) -> str:
    ...
    if not deliver_key or deliver_key == "cron":
        return ""
    hint = _resolve_platform_hint(
        agent,
        deliver_key,
        _default_platform_hint(deliver_key, getattr(agent, "valid_tool_names", None)),
    )
...
def platform_hint(agent: Any) -> str:
    ...
    _effective_hint = _resolve_platform_hint(
        agent,
        platform_key,
        _default_platform_hint(platform_key, getattr(agent, "valid_tool_names", None)),
    )
