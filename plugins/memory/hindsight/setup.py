...
def run_setup(provider, hermes_home: str, config: dict) -> None:
    ...
    if mode == "local_embedded":
        # The PM hindsight extra contains only hindsight-client; do not claim
        # that syncing it supplies the separate heavyweight embedded runtime.
        from .embedded import _check_local_runtime
        available, reason = _check_local_runtime()
        if available:
            print("  ✓ Embedded runtime available")
        else:
            print(f"  ⚠ Embedded runtime unavailable: {reason or 'missing dependency'}")
            print("  local_embedded requires hindsight-all; install it through an explicitly managed plugin dependency before activation.")
    else:
        from pm.client import sync_venv
        try:
            sync_venv(["hindsight"], explicit=True)
        except Exception as exc:
            print(f"  ⚠ Managed dependency install failed: {exc}")
            print("  Run manually: hermes pm install --extra hindsight")
        else:
