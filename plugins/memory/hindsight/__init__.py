...
def _ensure_client_dependency(module_name: str) -> None:
    """Resolve the optional client through PM, not the retired updater shim."""
    import importlib
    ...
    try:
        ...
        if exc.name != module_name:
            raise  # An installed SDK with a broken transitive dependency is not missing.
    if module_name != "hindsight_client":
        # Embedded mode has already passed _check_local_runtime(); it is not
        # part of the lightweight hindsight PM extra.
        raise ImportError("Hindsight embedded runtime unavailable (hindsight-all required)")
    from pm.extras import ensure_import

    try:
        ensure_import("hindsight")
    except Exception as exc:
        raise ImportError(f"Hindsight client unavailable: {exc}") from exc
...
def _maybe_upgrade_client() -> None:
    """Ask PM for the pinned Hindsight extra when an old client is detected."""
    ...
    try:
        ...
        return  # Missing clients are resolved on first use via ensure_import.
    ...
    try:
        ...
        logger.warning("hindsight-client %s is outdated; PM sync unavailable: %s. Run: hermes pm install --extra hindsight",
                       installed, exc)
